...
-- Enable RLS on storage.objects (it is enabled by default); create permissive policies limited to this bucket
create policy if not exists "Public read product images"
  on storage.objects for select
  using (bucket_id = 'product-images');

create policy if not exists "Public upload product images"
  on storage.objects for insert
  with check (bucket_id = 'product-images');

create policy if not exists "Public update product images"
  on storage.objects for update
  using (bucket_id = 'product-images')
  with check (bucket_id = 'product-images');

-- Optional: allow deletes (commented out by default)
-- create policy if not exists "Public delete product images"
